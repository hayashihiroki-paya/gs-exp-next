...
export default function LoginPage() {
  ...
  // State（状態）の定義
  // ========================================
  // 入力されたデータを操作するにはuseStateを使う方法のみ
  ...
  const handleAuth = async (e: React.FormEvent) => {
    // fromタグの送信（ボタンタグを押したときの挙動）の前にページがリロードされるのを防ぐ
    e.preventDefault();

    // エラーをクリア
    setError("");

    // ローディングを管理するuseStateをtrueにして今処理をしている状態にする
    setLoading(true);
    ...
    try {
      ...
      // finallyはすべて処理が終わった後必ず実行されるところ
      // 処理が終わったらローディングをfalseにしてボタンを有効にする
      setLoading(false);
    }
  }

  // returnより下はhtmlを書くところ
  return (
